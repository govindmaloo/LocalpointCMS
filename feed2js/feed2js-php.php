...
if (isset($restrict_url) && substr($src_host, strlen($src_host)-strlen($restrict_url)) != $restrict_url) {
	...
	$localpoint = false;
	if($rss->channel['link'] == "http://www.localpoint.ch"){
			$localpoint = true;
	}
		...
		foreach ( $all_items as $item ) {
			...
			elseif ($localpoint && $item['content']['encoded']) {
				// link url				
			...
			elseif ($item['title'] && $desc==0) {
				...
				//print $rel;
				$str.= "<li class=\"rss-item\"><a class=\"rss-item\" href=\"" . trim($my_url) . "\"" . $rel . $target_window . '>' . $my_title . '</a>' .  $author_str . "<br />";
			...
			if ($desc) {
			 ...
			 // Atom/encocded content support (thanks David Carter-Tod)
				...
				if ($html != 'a') { $my_blurb = strip_tags($my_blurb);}
						...
						preg_match_all('/(src|alt)=("[^"]*")/i',$img, $img_tag);
						//print $img
						//print_r($img_tag);
						if(strlen($img_tag[0][0]) > 0){						
							$src = strstr($img_tag[0][0], "blogger.googleusercontent.com", true);						
						}
						if($src){
							$img = "";
							$my_blurb = $img_tag['src'] .$my_blurb;
						}
		...
		$style=" ";
			    if(strlen($img) > 0){$str.= "<div class=rss-img>" . $img . "</div>"; $style='style="margin-left:80px;"'; $img ="";} 
				$my_title = addslashes(strip_returns($item['title']));
				$str.= "<div ".$style."><a class=\"rss-item\" href=\"" . trim($my_url) . "\"" . $rel . $target_window . '>' . $my_title . '</a>' .  $author_str . "<br />"; 
				$str.= addslashes(strip_returns($my_blurb, $br)) . "</div>"; 
