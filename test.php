...
//$dest   = '/home/httpd/vhosts/localpoint-web.ch/httpdocs/lpcms717/xml'; // Path to directory                 
$dest = 'xml/';
...
{ 
    ...
    function mysql_dump($dbUser, $dbPass, $dbName) 
    { 
        print $this->cmd = "mysql -X -u {$dbUser} -p{$dbPass} {$dbName} -e 'select * from users' > xml/gru.xml"; 
    } 
    function backup() 
    { 
        $xml = system($this->cmd, $retval); 			
        if(!empty($retval)) 
        { 
			//fopen('xml/gru.xml', 'x+')
            /*foreach($retval as $val){
				file_put_contents('file.txt',$val, FILE_APPEND);
			}*/
        } 
		print file_get_contents('xml/gru.xml');
    } 
} 
// initilize mysqldump 
$dump = new mysql_dump($dbUser, $dbPass, $dbName); 
// backup database 
$dump->backup(); 
 ?>
